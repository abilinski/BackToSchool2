...
s = p %>% filter(age >=5 & age < 11) %>% group_by(school_id) %>% summarize(num_HHs = length(unique(sp_hh_id)),
...
r = q %>% filter(age >=5 & age < 11) %>% group_by(sp_hh_id) %>% summarize(age_dist = paste(sort(age), collapse = ",")) %>%
...
save(synthpop, file = paste0("synth", state, ".RData"))
