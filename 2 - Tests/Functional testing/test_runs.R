  ...
  for(j in 0:23){
    ...
    foreach(i=start:end, .combine=rbind) %dopar% {
      ...
      out <- tryCatch(
        withCallingHandlers(sims(df, i, synthpop, class = class, job.no = job),
                            ...
                            save(stack, file = paste("/n/home00/jgiardina/BackToSchool2/3 - Output/Error Output/Technical Note/", paste("error", job, j, i, sep = "_"), ".RData", sep = ""))
                            return(NULL)}),
        error = function(c){
    ...
    output <- tryCatch(
      ...
      error = function(c){message("Original error message:")
        message(c)
        move_files(filelist, "/n/home00/jgiardina/BackToSchool2/3 - Output/Error Output/Technical Note")
        return(NULL)})
    setwd("/n/home00/jgiardina/BackToSchool2/3 - Output/Final Output/Technical Note")
...
df_ELEM.temp <- expand_grid(mitigation = c(0, 0.5),
                            prob = c(5, 25, 50)*3/100000) %>%
  mutate(start_type = "cont", n_HH = 2, test_days = "week",
         ...
         test_start_day = 1, n_tot = 1, isolate = T, no_test_vacc = F, time_seed_inf = NA, seed_asymp = F, surveillance = F, overdisp_off = T, num_adults = 2, version = 2, include_weekends = T, rel_trans_adult = 2,  rapid_test_sens = 0.8,
