...
initialize_school = function(n_contacts = 10, n_contacts_brief = 0, rel_trans_HH = 1, rel_trans_HH_symp_child = 1,
  ...
  df = start %>%
    mutate(id = row_number(),
           ...
           inf_days = 0,
           inf_home_days = 0,
           exposed_not.inf_days = 0,
           exposed_not.symp_days = 0,
           symp_days = 0,
           symp_and_inf_days = 0,
           last = 0,
           rapid_tp_count  = 0,
...
run_household = function(a, df, t){
  ...
  if(df$HH_id[df$id==a]>0 & sum(df$HH_id==df$HH_id[df$id==a])>1) {
    ...
    if(!(setequal(HH_vec$id[df$class_trans_prob[df$id==a]*df$relative_trans_HH[df$id==a]*HH_vec$susp*HH_vec$not_inf > 0], df$id[df$HH_id == df$HH_id[df$id==a] & df$id != a & (df$t_inf < 0 | df$t_exposed > t) & !df$now & df$susp != 0]))) stop("Incorrect individuals in/left out of household contacts")
  ...
  if(length(df$id[df$HH_id == df$HH_id[df$id==a] & df$id != a & (df$t_inf < 0 | df$t_exposed > t) & !df$now]) != 0) stop("No household members contacted by infected individual")}
...
#'
#' @export
run_class = function(a, df, high_school = F, hs.classes = NA, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf){
  ...
  if(df$class[df$id==a]!=99 & !high_school) {
    ...
    if(!(setequal(class_vec$id[df$class_trans_prob[df$id==a]*class_vec$susp*class_vec$present_susp > 0], df$id[df$class == df$class[df$id==a] & df$id != a & (df$t_inf < 0 | df$t_exposed > t) & !df$now & df$susp != 0 & df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)]))){
      message("Incorrect individuals in/left out of classroom contacts")
      print("Time:")
      print(t)
      print("present_susp in class_vec:")
      print(class_vec[class_vec$present_susp, c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present", "present_susp", "t_exposed", "not_inf", "location", "now", "source")], n = Inf)
      print("Expected classroom contacts")
      print(df[df$class == df$class[df$id==a] & df$id != a & (df$t_inf < 0 | df$t_exposed > t) & !df$now & df$susp != 0 & df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf), c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present", "present_susp", "t_exposed", "not_inf", "location", "now", "source")], n = Inf)
      print("All classroom individuals:")
      print(df[df$class == df$class[df$id==a] & df$id != a, c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present", "present_susp", "t_exposed", "not_inf", "location", "now", "source")], n = Inf)
      print("Household members of extras:")
      print(df[df$HH_id %in% df$HH_id[df$id %in% setdiff(df$id[df$class == df$class[df$id==a] & df$id != a & (df$t_inf < 0 | df$t_exposed > t) & !df$now & df$susp != 0 & df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)], class_vec$id[class_vec$present_susp])], c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present", "present_susp", "t_exposed", "not_inf", "location", "now", "source")])
      print("All infected individuals:")
      print(df[df$t_end_inf_home >= 0, c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present", "present_susp", "t_exposed", "not_inf", "location", "now", "source")], n = Inf)
    ...
    infs = 0
    if(length(df$id[df$class == df$class[df$id==a] & df$id != a & df$class != 99]) != 0){
      message("No classroom members contacted by infected individual")
      print("Number of individuals in classroom (besides infected individual):")
      print(length(df$id[df$class == df$class[df$id==a] & df$id != a & df$class != 99]))
      stop("See message above")
    }
  }
...
#'
#' @export
run_rand = function(a, df, random_contacts, check.params, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf){
  ...
  if(nrow(contacts) != check.params[["n_contacts"]]) stop(paste("Number of random contacts for an infected individual is", nrow(contacts), "rather than", check.params[["n_contacts"]]))
  ...
  if(!all(contacts$id[df$class_trans_prob[df$id==a]*df$relative_trans[df$id==a]*contacts$susp*contacts$present_susp > 0] %in% df$id[(df$t_inf < 0 | df$t_exposed > t) & !df$now & df$susp != 0 & df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)])) stop("Incorrect individuals in/left out of random contacts")
...
#'
#' @export
run_staff_rand = function(a, df, n_contact, rel_trans_adult = 2, check.params, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf){
  
  if(n_contact>0){
    # pull contacts from random graph
    tot = length(df$id[df$present & df$adult & !df$family & df$id != a])
    contact_take = ifelse(n_contact<=tot, n_contact, tot)
    contact_id = sample(df$id[df$present & df$adult & !df$family & df$id != a], contact_take)
    contacts = df[df$id %in% contact_id & df$id!=a,]

  if(nrow(contacts) != check.params[["n_staff_contact"]] & length(df$id[df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf) & df$id != a & df$adult]) >= check.params[["n_staff_contact"]]){
      stop(paste("Number of random staff contacts for an infected individual is", nrow(contacts), "rather than", check.params[["n_staff_contact"]])) 
    }
    ...
    if(!all(contacts$id[df$class_trans_prob[df$id==a]*df$relative_trans[df$id==a]*contacts$susp*contacts$present_susp*rel_trans_adult > 0] %in% df$id[(df$t_inf < 0 | df$t_exposed > t) & !df$now & df$susp != 0 & df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)])) stop("Incorrect individuals in/left out of random staff contacts")
...
#'
#' @export
run_care = function(a, df, care_contacts, rel_trans_CC = 2, num_adults = 2, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf, n_HH){
  ...
  if((!df$adult[df$id==a] | a %in% keep)){
    ...
    group.size <- care_contacts %>% group_by(cat) %>% summarise(size = n())

    if(length(unique(contacts$HH_id[contacts$HH_id != df$HH_id[df$id == a]])) != n_HH - 1){
      
      if(length(unique(group.size$cat[which.min(group.size$size)])) > 1) stop("More than one household care group less than size n_HH")
      
      if(!(care_contacts$cat[care_contacts$HH_id == df$HH_id[df$id == a]] %in% unique(group.size$cat[which.min(group.size$size)]))) stop("Incorrect number of households contacted for care")
      }
    if(sum(contacts$adult) + length(df$id[df$HH_id %in% df$HH_id[df$id == a] & df$id %in% keep]) != num_adults){
      if(!(care_contacts$cat[care_contacts$HH_id == df$HH_id[df$id == a]] %in% unique(group.size$cat[which.min(group.size$size)])) | min(group.size$size)*2 != sum(contacts$adult) + length(df$id[df$HH_id %in% df$HH_id[df$id == a] & df$id %in% keep])) stop("Incorrect number of adults in care contacts")
    }
    ...
    if(!all(contacts$id[df$class_trans_prob[df$id==a]*contacts$susp*round(contacts$not_inf)*rel_trans_CC > 0] %in% df$id[(df$t_inf < 0 | df$t_exposed > t) & !df$now & df$susp != 0 & df$id %in% check.present.care(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)])) stop("Incorrect individuals in/left out of care contacts")
...
#'
#' @export
run_specials = function(a, df, specials, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf){
  ...
  if(!df$adult[df$id==a] & df$class[df$id==a]%in%specials$class){
    ...
    prob_specials = rbinom(nrow(specials_vec), size = 1, prob = ifelse(df$class_trans_prob[df$id==a]*df$relative_trans[df$id==a]*specials_vec$susp*specials_vec$present_susp < 1,
                                                              ...
                                                              if(!setequal(specials_vec$id[df$class_trans_prob[df$id==a]*df$relative_trans[df$id==a]*specials_vec$susp*specials_vec$present_susp > 0], df$id[(df$t_inf < 0 | df$t_exposed > t) & !df$now & df$susp != 0 & df$id %in% specials$teacher[specials$class == df$class[df$id == a]] & df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)])) stop("Incorrect individuals in/left out of specials contacts")
    ...
    if(!setequal(specials_class_vec$id[df$class_trans_prob[df$id==a]*df$relative_trans[df$id==a]*specials_class_vec$susp*specials_class_vec$present_susp > 0], df$id[(df$t_inf < 0 | df$t_exposed > t) & !df$now & df$susp != 0 & df$id %in% df$id[df$class %in% specials$class[specials$teacher == a]] & df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)])) stop("Incorrect individuals in/left out of specials contacts")
    
  } else{
    infs = 0
    if(df$class[df$id == a] %in% specials$class & !df$adult[df$id == a]) stop("Infected individual has specials class scheduled for today, but has not contacted anyone.")
  }
...
check.present.school <- function(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf){
  
  quarantine.frame_symp <- df[ceiling(df$t_symp) + turnaround.time <= t & !df$sub_clin & df$isolate & df$class != 99 & df$t_symp + turnaround.time >= time_seed_inf & (ceiling(df$t_symp) <= df$test_time | df$test_time < 0),]
  
  quarantine.frame_test <- df[df$test_time + turnaround.time <= t & df$test_time >= 0 & df$class != 99,]
  
  quarantine.frame_symp$begin <- ceiling(quarantine.frame_symp$t_symp)
  quarantine.frame_test$begin <- quarantine.frame_test$test_time
  
  quarantine.frame <- rbind(quarantine.frame_symp, quarantine.frame_test)
  
  quarantine.frame$end <- quarantine.frame$begin + turnaround.time + quarantine.length + quarantine.grace
  
  quarantine.frame <- quarantine.frame[order(quarantine.frame$class, quarantine.frame$begin),]
  quarantine.frame$active <- FALSE
  
  quarantine.tracker <- c()
  
  for(class in unique(quarantine.frame$class)){
    quarantine.class <- quarantine.frame[quarantine.frame$class == class,]
    for(i in 1:nrow(quarantine.class)){
      if(i == 1){
        quarantine.class$active[i] <- TRUE
        quarantine.tracker <- c(quarantine.tracker, paste(quarantine.class$id[i], quarantine.class$begin[i]))
        #print(quarantine.tracker)
      } else if(quarantine.class$begin[i] + turnaround.time > max(quarantine.class$end[quarantine.class$active])){
        quarantine.class$active[i] <- TRUE
        quarantine.tracker <- c(quarantine.tracker, paste(quarantine.class$id[i], quarantine.class$begin[i]))
        #print(quarantine.tracker)
      }else{
        quarantine.class$active[i] <- FALSE
      }
    }
  }
  
  quarantine.list <- quarantine.frame$class[paste(quarantine.frame$id, quarantine.frame$begin) %in% quarantine.tracker & (quarantine.frame$begin + turnaround.time <= t & quarantine.frame$end - quarantine.grace - 1 >= t & quarantine.frame$class != 99 & quarantine.frame$begin + turnaround.time >= time_seed_inf)]
  
  #Exclude family members
  df <- df[!df$family,]
  
  #Exclude people in quarantined classes (or if in the household of someone who is quarantined) if test-to-stay is off and notify is on
  ##Also exclude people on their fist day of symptoms
  if(!test_quarantine & df$notify[1]){
    df <- df[(!(df$class %in% quarantine.list) | df$vacc) & !(df$HH_id %in% df$HH_id[df$class %in% quarantine.list & !df$vacc]),]
  }
  
  #Only include people scheduled to be in school
  df <- df[df$id %in% sched$id[sched$present & sched$t == t],]
  
  #Exclude people who have tested positive through screening
  df <- df[!(df$test_time + turnaround.time <= t & df$test_time >= 0),]
  
  #Exclude people who are symptomatic (when isolate is on) (people attend school on first day of symptoms if test-to-stay is on)
  df <- df[!(df$t_symp <= t & !df$sub_clin & df$isolate) | is.na(df$sub_clin),]
  
  #Exclude people who have tested positive on a test-to-stay test, or live in a household with someone who had a test-to-stay test today
  df <- df[!(df$test_time_q <= t & df$test_time_q >= 0) & !(df$HH_id %in% df$HH_id[df$test_time_q == t]),]
  
  return(df$id)
}

check.present.care <- function(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf){
  ...
  #Exclude people in quarantined classes (even when test-to-stay is on)
  if(df$notify[1]){
    df <- df[(!(df$class %in% quarantine.list) & !(df$HH_id %in% df$HH_id[df$class %in% quarantine.list])),]
  }
  
  #Only include students and family
  df <- df[!df$adult | df$family,]
  
  #Only include people not scheduled to be in school
  df <- df[!(df$id %in% sched$id[sched$present & sched$t == t]),]
  
  #Exclude people who have tested positive through screening
  df <- df[!(df$test_time + turnaround.time <= t & df$test_time >= 0),]
  
  #Exclude people who are symptomatic (when isolate is on)
  df <- df[!(df$t_symp <= t & !df$sub_clin & df$isolate) | is.na(df$sub_clin),]
  
  #Exclude people who have tested positive on a test-to-stay test, or live in a household with someone who had a test-to-stay test positive today
  df <- df[!(df$test_time_q <= t & df$test_time_q >= 0) & !(df$HH_id %in% df$HH_id[df$test_time_q == t]),]
...
run_model = function(time = 30,
  ...
  for(t in time_seed_inf:(time_seed_inf+time-1)){
    ...
    if(t != time_seed_inf){
      if(any(df$t_exposed[df$now] != t - 1)) stop("Individuals infected in last time period not run through make_infected") 
    }
    ...
    if(test_quarantine==T) {
      ...
      ##For later functional testing
      df$test_time_q = ifelse(df$test_q & !(df$test_time_q > 0) & df$t_inf <= t & df$t_end_inf_home >= t & !df$family & df$id %in% sched$id[sched$present & sched$t == t] & (df$t_symp >= t | (df$symp & df$sub_clin) | !df$isolate), t, df$test_time_q)
      ####
    ...
    df$exposed_not.inf_days = df$exposed_not.inf_days + as.numeric(!df$start & df$t_inf > t & df$t_exposed <= t & df$t_end_inf_home >= t)
    df$exposed_not.symp_days = df$exposed_not.symp_days + as.numeric(!df$start & !df$family & ifelse(is.na(df$sub_clin), TRUE, !df$sub_clin) & ifelse(is.na(df$symp), FALSE, df$symp) & df$t_exposed <= t & df$t_symp > t & df$t_end_inf_home >= t)
    ...
    if(test & t%in%testing_days){
      ...
      #Throw error if testing is not on a Monday
      if(!(sched$day[sched$t==t][1] %in% c("M"))) stop("Testing not on Monday")
      ...
      ##For later functional testing
      df$test_time = ifelse(df$test & !(df$test_time > 0) &
                              df$t_inf <= t & df$t_end_inf_home >= t &
                              df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf), t, df$test_time)
      ####
    ...
    if(sum(df$inf_home)>0) {
      ...
      if(!(setequal(home_infs, df$id[df$t_inf <= t & df$t_end_inf_home >= t & !(df$start | df$location == "")]))) stop("Incorrect individuals in/left out of home_infs")
      
      for(a in home_infs){
        
        #Check length of infectiousness
        df$inf_home_days[df$id == a] = df$inf_home_days[df$id == a] + 1
        
        # HOUSEHOLD CONTACTS
        inf_vec = run_household(a, df, t)
        ...
        #Test to ensure community sourced infections not transmitted within the household
        
        if((df$location[df$id == a] == "" | df$start[df$id == a]) & (!(0 %in% inf_vec) | length(inf_vec) > 1)) stop("Community sourced infections transmitted within the household")
    ...
    if(sum(df$trans_now)>0) {
      ...
      #Throw error if someone is infectious at school on a weekend
      if(sched$day[sched$t==t][1] %in% c("Sa", "Su") & sum(df$trans_now)>0) stop("Infectious individuals at school on weekend")
      ...
      if(!all((specials %>% count(teacher))$n == 4)) stop("Number of specials classes assigned to each teacher not equal to 4")
      ...
      if(!(setequal(school_infs, df$id[df$t_inf <= t & df$t_end_inf_home >= t & df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)]))){
        message("Incorrect individuals in/left out of school_infs")
        print("Time:")
        print(t)
        print("school_infs:")
        print(df[df$id %in% school_infs, c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present")])
        print("Expected infectious individuals:")
        print(df[df$t_inf <= t & df$t_end_inf_home >= t &
                   df$id %in% check.present.school(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf), c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present")])
        print("All infectious individuals:")
        print(df[df$t_end_inf_home > 0, c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present")], n = Inf)
        print(paste("Schedule for time", t))
        print(sched[sched$t == t & sched$id %in% df$id[df$t_inf <= t & df$t_end_inf_home > 0],], n = Inf)
        print(classes_out)
        print(df[df$class %in% classes_out$class, c("id","class","HH_id")], n = Inf)
        print(df[(ceiling(df$t_symp) + turnaround.time <= t & !df$sub_clin & df$isolate & df$class != 99 & df$t_symp + turnaround.time >= time_seed_inf) | (df$test_time + turnaround.time <= t & df$test_time >= 0 & df$class != 99),], n = Inf)
        print(df[(ceiling(df$t_symp) + turnaround.time <= t & ceiling(df$t_symp) + turnaround.time + quarantine.length - 1 >= t & !df$sub_clin & df$isolate & df$class != 99 & df$t_symp + turnaround.time >= time_seed_inf) | (df$test_time + turnaround.time <= t & df$test_time + turnaround.time + quarantine.length - 1 >= t & df$test_time >= 0 & df$class != 99),], n = Inf)
      ...
      for(a in school_infs){
        ...
        # CLASS CONTACTS
        class_trans = run_class(a, df, high_school = high_school, hs.classes = hs.classes, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)
        #class_trans = 0
        ...
        # RANDOM CONTACTS
        rand_trans = tryCatch({run_rand(a, df, random_contacts, check.params, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)}, error = function(err) {0})
        #rand_trans = 0
        ...
        if(df$adult[df$id==a] & !df$family[df$id==a]){
          rand_staff_trans.out = run_staff_rand(a, df, n_staff_contact, rel_trans_adult, check.params, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)
        ...
        # SPECIALS CONTACTS
        specials_trans = run_specials(a, df, specials, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)
        #specials_trans = 0
    ...
    if(sum(df$trans_outside)>0 & n_HH>0 & len > 1 &
      ...
      if(!(setequal(non_school_infs, df$id[df$t_inf <= t & df$t_end_inf_home >= t & df$id %in% check.present.care(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf)]))){
        message("Incorrect individuals in/left out of non_school_infs")
        print("Time:")
        print(t)
        print("non_school_infs:")
        print(df[df$id %in% non_school_infs, c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present")])
        print("Expected infectious individuals:")
        print(df[df$t_inf <= t & df$t_end_inf_home >= t & df$id %in% check.present.care(df, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf),c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present")])
        print("All infectious individuals:")
        print(df[df$t_end_inf_home >= 0,c("id", "t_inf", "t_end_inf_home", "t_symp", "symp", "sub_clin", "adult", "family", "HH_id", "class", "test_time", "test_time_q", "vacc", "t_end_inf", "t_notify", "q_out", "detected", "inf", "present")], n = Inf)
        print("Classes out:")
        print(classes_out)
      ...
      for(a in non_school_infs){
        ...
        # CARE CONTACTS
        care_trans.out = run_care(a, df, care_contacts, rel_trans_CC, num_adults = num_adults, sched, t, turnaround.time, quarantine.length, quarantine.grace, test_quarantine, time_seed_inf, n_HH)
  ...
  #Add remaining days to checks
  df$exposed_not.inf_days = df$exposed_not.inf_days + ifelse(!df$start & df$t_inf > time_seed_inf+time-1, floor(df$t_inf - (time_seed_inf+time-1) - 1e-10), 0)
  df$exposed_not.symp_days = df$exposed_not.symp_days + ifelse(!df$start & df$t_symp > time_seed_inf+time-1 & !df$family & ifelse(is.na(df$sub_clin), TRUE, !df$sub_clin) & ifelse(is.na(df$symp), FALSE, df$symp), floor(df$t_symp - (time_seed_inf+time-1)), 0)
  df$inf_home_days = df$inf_home_days + ifelse(!df$start & df$t_end_inf_home > time_seed_inf+time-1, pmin(floor(df$t_end_inf_home - (time_seed_inf+time-1)), c(ceiling(df$t_end_inf_home) - ceiling(df$t_inf))), 0)
...
mult_runs = function(N, n_other_adults, n_contacts, rel_trans_HH,
                     ...
                     adult_unvax_hosp_rate, child_unvax_hosp_rate, adult_vax_hosp_rate, child_vax_hosp_rate, table1_checks,
                     version = 2){
  ...
  ####Set up list of parameters for functional testing
  check.params <- c("n_contacts" = n_contacts, "n_staff_contact" = n_staff_contact)
  ...
  for(i in 1:N){
    ...
    df = run_model(time = time, notify = notify, test = test, df = school, sched = sched,
                   ...
                   rapid_test_sens = rapid_test_sens, overdisp_off = overdisp_off, check.params = check.params)
    ...
    if(!run_specials_now & keep$related_arts[i] > 0) stop("Infections occured in specials classes with run_specials_now off")
    ...
    keep$length.infectious_obs[i] = mean(ceiling(df$t_end_inf_home[df$t_inf > 0]) - ceiling(df$t_inf[df$t_inf > 0]))
    keep$length.infectious_sd[i] = sd(ceiling(df$t_end_inf_home[df$t_inf > 0]) - ceiling(df$t_inf[df$t_inf > 0]))
    ...
    keep$exposed_not.inf_days[i] = sum(df$exposed_not.inf_days[!df$start & df$t_exposed >= time_keep])
    keep$num_exposed[i] = sum(!df$start & df$t_exposed >= time_keep)
    keep$exposed_not.symp_days[i] = sum(df$exposed_not.symp_days[!df$start & !df$family & ifelse(is.na(df$sub_clin), TRUE, !df$sub_clin) & df$t_exposed >= time_keep & ifelse(is.na(df$symp), FALSE, df$symp)])
    keep$num_symp = sum(!df$start & !df$family & ifelse(is.na(df$sub_clin), TRUE, !df$sub_clin) & df$t_exposed >= time_keep & ifelse(is.na(df$symp), FALSE, df$symp))
    keep$inf_home_days[i] = sum(df$inf_home_days[!df$start & df$t_inf >= time_keep])
    keep$num_inf = sum(!df$start & df$t_inf >= time_keep)
    ...
    keep$child.community.risk.days <-
      length(df$id[df$t_inf <= 0 & !df$adult])*(df$start.time[1] + time) +
      length(df$id[df$start & !df$adult])*(df$start.time[1] + time) +
      sum(df$t_exposed[!df$start & !df$adult & df$t_inf > 0])
    keep$child.start.count <- sum(df$start[!df$adult])
    
    keep$adult.community.risk.days <-
      length(df$id[df$t_inf <= 0 & df$adult])*(df$start.time[1] + time) +
      length(df$id[df$start & df$adult])*(df$start.time[1] + time) +
      sum(df$t_exposed[!df$start & df$adult & df$t_inf > 0])
    keep$adult.start.count <- sum(df$start[df$adult])
