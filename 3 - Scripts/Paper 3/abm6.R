...
initialize_school = function(n_contacts = 10, n_contacts_brief = 0, rel_trans_HH = 1,
                             ...
                             isolate = T, dedens = T, run_specials = F, start, vax_eff = .9){
...
run_model = function(time = 30,
  ...
  # testing
  if(test_type=="all"){ df$test_type = !df$family & df$vacc!=1
  ...
  for(t in time_seed_inf:(time_seed_inf+time-1)){
    ...
    df$symp_now = !df$family & df$t_symp > -1 & df$t_symp <= t & df$t_end_inf >=t
    df$test_ct_q = df$test_ct_q + df$class%in%classes_out$class
  
    # present
    if(test_quarantine==T) {
    df$test_type_q = !df$family & df$vacc!=1 & ((df$class%in%classes_out$class & (df$group%in%classes_out$group | df$group==99)))
    df$test_ct_q = df$test_ct_q + df$present*as.numeric(df$test_type_q)
    df$test_q = rbinom(nrow(df), size = 1, prob = 0.8*df$present*as.numeric(df$test_type_q))
    ...
    df$detected_q = ifelse(df$inf & df$test_q & df$present, 1, df$detected)
...
mult_runs = function(N = 500, n_other_adults = 30, n_contacts = 10, n_contacts_brief = 0, rel_trans_HH = 1,
                     ...
                     bubble = F, include_weekends = T, turnaround.time = 1, test_start_day = 1, test_quarantine = F, vax_eff = 0.9, version = 2){
  ...
  for(i in 1:N){
    ...
    school = initialize_school(n_contacts = n_contacts, n_contacts_brief = n_contacts_brief, rel_trans_HH = rel_trans_HH,
                               ...
                               isolate = isolate, dedens = dedens, run_specials = run_specials_now, start = class, vax_eff = vax_eff)
