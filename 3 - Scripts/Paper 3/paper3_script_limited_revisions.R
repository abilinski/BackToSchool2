...
sims = function(df, i, synthpop, class = NA, job.no = "0"){
  ...
  out = mult_runs(version = 2, N = df$n_tot[i], n_contacts = df$n_contacts[i], n_staff_contact = df$n_staff_contact[i],
                  ...
                  test_days = df$test_days[i], test_type = df$test_type[i], rel_trans_HH = 2*df$variant.attack[i]/df$attack.df[i], rel_trans_HH_child = df$rel_trans_child_HH[i],
                  rel_trans_CC = df$variant.attack[i]/df$attack.df[i], n_HH = df$n_HH[i],
                  quarantine.length = df$quarantine.length[i], turnaround.time = df$turnaround.time[i],
                  test_start_day = df$test_start_day[i], family_susp = df$family_susp[i], test_quarantine = df$test_quarantine[i], vax_eff = df$vax_eff[i], child_vax = df$child.vax[i])
  out[,c("id")] <- id.number
  out <- bind_cols(out, df[i,])
    ...
    foreach(i=start:end, .combine=rbind) %dopar% {
      ...
      out <- tryCatch(
        withCallingHandlers(sims(df, i, synthpop, class = class, job.no = job),
                                          ...
                                          save(stack, file = paste("/n/home00/jgiardina/BackToSchool2/3 - Output/Error Output/Round 3/", paste("error", job, j, i, sep = "_"), ".RData", sep = ""))
                                          return(NULL)}),
                      error = function(c){
    ...
    output <- tryCatch(
      rbindlist(mclapply(1:length(filelist), function(a){load(filelist[a]); out = out; return(out)}, mc.cores = detectCores())),
                       error = function(c){message("Original error message:")
                         message(c)
                         move_files(filelist, "/n/home00/jgiardina/BackToSchool2/3 - Output/Error Output/Round 3")
                         return(NULL)})
    setwd("/n/home00/jgiardina/BackToSchool2/3 - Output/Final Output/Revisions Round 6")
...
df_ELEM = expand_grid(mitigation = seq(from = 0, to = 1, by = 0.01),
  ...
  mutate(start_type = "cont", n_HH = 2, test_days = "week",
         ...
         n_staff_contact = 10, rel_trans_child_HH = 1/child_trans, rel_trans = 1/8, start_mult = 0, disperse = T, 
