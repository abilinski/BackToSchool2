    ...
    output <- tryCatch(
      rbindlist(mclapply(1:length(filelist), function(a){load(filelist[a]); out = out; return(out)}, mc.cores = detectCores())),
                       error = function(c){message("Original error message:")
                         ...
                         return(NULL)})
    setwd("/n/home00/jgiardina/BackToSchool2/3 - Output/Final Output/JNO Full Paper")
...
df_ELEM.delta.low.vax.eff =  df_ELEM.temp %>%
  expand_grid(type = c("base"),
              variant.attack = c(0.07),
...
df_ELEM <- rbind(df_ELEM.delta, df_ELEM.delta.low.vax.eff, df_ELEM.high_vax_coverage, df_ELEM.prev.variants)
