...
# simulation setup
df_ELEM = make_df(attack = 0.02, n_tot = n_tot, start_type = "cont", n_HH = 2,
...
df_ELEM_SURV = make_df(n_tot = n_tot*5, start_type = "cont", n_HH = 2,
                       test_days = c("week"), 
                       test_type = c("all"),
                       test_frac = c(0.1, 0.2, .9),
                       test_sens = .9, vax_eff = c(.8),
                       scenario = c("Base case"), teacher_susp = c(.9),
                       prob = c(1,5,10,25,50,100)*3/100000, time = 30,
                       child_trans = .5, child_susp = .5, high_school = F,
                       p_asymp_adult = .2, p_asymp_child = 0,
                       p_subclin_adult = .2, p_subclin_child = .8,
                       mult_asymp = .5, quarantine.length = c(7, 10), attack = c(0.02, 0.04),
                       turnaround.time = c(2),
                       n_other_adults = 30, n_class = 5, 
                       test_quarantine = test_q, notify = notify_val, 
                       test = c(F,T), surveillance = T) %>%
  filter(!(test_frac==0.9 & attack!=0.04))
df_ELEM_SURV2 = df_ELEM_SURV %>% mutate(test = F) %>% filter(attack==0.04 & test_frac==0.9)

df_ELEM_ADD = make_df(attack = .02, n_tot = n_tot*2, start_type = "cont", n_HH = 2,
                      test_days = c("week", "2x_week"),
                      test_type = c("all"),
                      test_frac = c(.7, .9),
                      test_sens = .9,
...
df_ELEM = rbind(df_ELEM, df_ELEM_SURV, df_ELEM_SURV2, df_ELEM_ADD, df_ELEM1, df_ELEM2, df_SENS2, df_SENS3, df_SENS4, df_SENS5) %>% mutate(i = row_number()) %>% 
