...
graph_sims1 = function(sims, title = "", palette = pal, ymax = NA){
  ...
  a = ggplot(out, 
    ...
    geom_text(size = font-1, aes(label = case_when(mean.new>10~round(mean.new), mean.new <10 & mean.new > 1~round(mean.new, 1), mean.new<1~round(mean.new, 2)))) + theme_opts + labs(x = "", y = "", title = title) + ylim(0, ymax)
...
graph_sims3 = function(sims, n_teacher = 1, n_student = 1, ymax = NA){
  ...
  plot_out2 = ggplot(out2 %>% filter(id_cat%in%c("5-day", "A/B") & attack_level=="Medium" & adult_prob %in% c(10, 50, 100)), 
                    aes(x = n_HH, y = value, group = paste(adult_prob, var, attack_level, id_cat, strategy),
                        col = as.character(adult_prob), lty = id_cat)) + geom_line() + 
  ...
  return(list(plot_out, out, plot_out2))
...
# make graphs
esa = graph_sims1(elem_s1, title = "A - children have equal infectiousness")
esb = graph_sims1(elem_s2, title = "A - with additional schedules", palette = pal2)
esc = graph_sims1(elem_s3, title = "A - children have overdispersed transmission")
esd = graph_sims1(elem_s5, title = "A - teacher as index case")
...
hsa = graph_sims1(high_s1, title = "B - adolescents less susceptible", ymax = 80)
hsb = graph_sims1(high_s2, title = "B - with different schedules", palette = pal2, ymax = 80)
hsc = graph_sims1(high_s3, title = "B - adolescents have overdispersed transmission", ymax = 80)

# DYNAMIC ELEMENTARY
load("fig_output_Dynamic Elem.RData")

a3 = graph_sims3(sims, n_student = 638, n_teacher = 60, ymax = .25)
sims1 = sims
load("fig_output_Dynamic Elem Sens.RData")
a4 = graph_sims3(bind_rows(sims, sims1), n_student = 638, n_teacher = 60, ymax = .25)
...
b4 = graph_sims3(bind_rows(sims, sims1), n_student = 1451, n_teacher = 124, ymax = 1)
...
# Paragraph 1
a[[4]] %>% filter(id2=="5-day" & strategy=="Symptomatic isolation") %>% dplyr::select(mean.new)
a[[4]] %>% filter(id2=="A/B" & strategy=="Symptomatic isolation") %>% dplyr::select(mean.new)
...
a[[4]] %>% filter(id2=="5-day" & strategy=="Symptomatic isolation") %>% dplyr::select(perc.zero, mt_5, mt_5_avg)
b[[4]] %>% filter(id2=="5-day" & strategy=="Symptomatic isolation") %>% dplyr::select(perc.zero, mt_5, mt_5_avg)
